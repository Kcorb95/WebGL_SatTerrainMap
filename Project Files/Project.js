        ...
        for (var j = 0; j < DEMObj.nrows; j++) {
            ...
            if ((i == 0 && j == 0) || (i == (DEMObj.ncols - 1) && j == 0) || (i == 0 && j == (DEMObj.nrows - 1)) || (i == (DEMObj.ncols - 1) && j == (DEMObj.nrows - 1))) {
                if ((i == 0 && j == 0)) {
                    ...
                    var t1 = subtract(vertices[i] * DEMObj.nrows + vertices[j + 1], vertices[i] * DEMObj.nrows + vertices[j]);//top of corner
                    var t2 = subtract(vertices[i + 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//right of corner
                } else if ((i == (DEMObj.ncols - 1) && j == 0)) {
                    //bottom right corner ncol,0
                    var t1 = subtract(vertices[i - 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//left of corner
                    var t2 = subtract(vertices[i] * DEMObj.nrows + vertices[j + 1], vertices[i] * DEMObj.nrows + vertices[j]);//top of corner
                } else if ((i == 0 && j == (DEMObj.nrows - 1))) {
                    //top left corner 0,nrow
                    var t1 = subtract(vertices[i] * DEMObj.nrows + vertices[j - 1], vertices[i] * DEMObj.nrows + vertices[j]);//bottom of corner
                    var t2 = subtract(vertices[i + 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//right of corner
                } else if ((i == (DEMObj.ncols - 1) && j == (DEMObj.nrows - 1))) {
                    //ncols,nrow corner top right
                    var t1 = subtract(vertices[i - 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//left of corner
                    var t2 = subtract(vertices[i] * DEMObj.nrows + vertices[j - 1], vertices[i] * DEMObj.nrows + vertices[j]);//bottom of corner
                }
                console.log("t1: " + t1);
                console.log("t2: " + t2);
                ...
                //interior vertex
                var t1 = subtract(vertices[i] * DEMObj.nrows + vertices[j + 1], vertices[i] * DEMObj.nrows + vertices[j]);//top of middle
                var t2 = subtract(vertices[i - 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//left of middle
                var t3 = subtract(vertices[i] * DEMObj.nrows + vertices[j - 1], vertices[i] * DEMObj.nrows + vertices[j]);//bottom of middle
                var t4 = subtract(vertices[i + 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//right of middle
                ...
                if (i == 0) {
                    //Left Edge
                    var t1 = subtract(vertices[i] * DEMObj.nrows + vertices[j - 1], vertices[i] * DEMObj.nrows + vertices[j]);//bottom of middle
                    var t2 = subtract(vertices[i + 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//right of middle
                    var t3 = subtract(vertices[i] * DEMObj.nrows + vertices[j + 1], vertices[i] * DEMObj.nrows + vertices[j]);//top of middle
                ...
                if (j == DEMObj.nrows - 1) {
                    //Top Edge
                    var t1 = subtract(vertices[i - 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//left of middle
                    var t2 = subtract(vertices[i] * DEMObj.nrows + vertices[j - 1], vertices[i] * DEMObj.nrows + vertices[j]);//bottom of middle
                    var t3 = subtract(vertices[i + 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//right of middle
                } else if (j == 0) {
                    //Bottom Edge
                    var t1 = subtract(vertices[i + 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//right of middle
                    var t2 = subtract(vertices[i] * DEMObj.nrows + vertices[j + 1], vertices[i] * DEMObj.nrows + vertices[j]);//top of middle
                    var t3 = subtract(vertices[i - 1] * DEMObj.nrows + vertices[j], vertices[i] * DEMObj.nrows + vertices[j]);//left of middle
                }
                normal1 = cross(t1, t2);
                normal2 = cross(t2, t3);
                normal = normalize(add(normal1, normal2));
