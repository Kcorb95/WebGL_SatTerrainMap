...
Profile.prototype.push = function(elev){
    this.points.push(vec3(
        this.coords[0] - Math.floor(this.dem.xsw),
        elev,
        this.coords[1] - Math.floor(this.dem.ysw-2) + (this.dem.yres*this.points.length)
    ));
}
...
function readRecA(reader, file, data, callback){
    ...
    reader.onload = function(evt) {
      ...
      if (data.length) { readRecA(reader, file, data, callback); }
      else { readRecB(reader, file, datum.endByte, callback); }
    };
...
function readRecB(reader, file, prevEnd, callback) {
    ...
    reader.onload = function(evt) {
        var tokens = evt.target.result.split(' ').filter(function(x){return x.length>0;});
        var values = tokens.map(function(x){return parseFloat(x.replace("D+","E+"));});
        ...
        if (colnum < DEMObj.ncols - 1) {
            ...
            DEMObj.heights.push([]);
            readElevs(file, colnum, endPos, colsize, callback, reader);
        } else {
...
function readElevs(file, i, curPos, numLeft, callback, reader) {//had to pass in reader
    ...
    reader.onload = function(evt) {
        ...
        if (i + 1 < DEMObj.ncols) { readRecB(reader, file, endPos, callback); }
        else { doneReading(callback); }
    };
...
function doneReading(callback) {
    //var maxLen = DEMObj.profiles.reduce(function(acc,x){return Math.max(acc,x.points.length);}, 0);
...
function blockStart(curPos) { return 1024 * Math.floor(curPos / 1024); }

function nextBlockStart(curPos) { return (curPos % 1024 === 0 ? curPos : (curPos + (1024 - curPos % 1024))); }
